...
import {IONFTSpecKX} from "./IONFTSpecKX.sol";
    ...
    Ownable2StepUpgradeable
{
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return
            bytes(globalInfo.baseUri).length > 0 ? string(abi.encodePacked(globalInfo.baseUri, tokenId.toString())) : "";
    }
    ...
    function setActiveSigners(address[] calldata signers, bool status) external onlyAdminOrOwner {
        uint256 len = signers.length;
        for(uint256 i = 0; i < len; i++){
            setActiveSigner(signers[i], status);
        }
    }

    function setActiveSigner(address signer, bool status) public onlyAdminOrOwner {
    ...
    function isStageActive(string calldata stage) public view returns (bool) {
        return uint256(stageToMint[stage].startTime) <= block.timestamp
